...
let funcFilteredArray = arrWithStrings.filter((elem) => {
    if (elem.length > 5) {
        return elem;
    }
});
console.log(funcFilteredArray);
...
//! let funcSuper = (array, repeated) => {
//     for (let i = 0; i < array.length; i++) {
//         if (array[i].includes(repeated)) {
//             return `число ${array[i]} самоое повторяющее, общее количество составило - ${array[i].length}`;
//         }
//     }
// };
// console.log(funcSuper([4, 5, 5, 5, 3, 5, 2, 3, 1, 3, 4], 5));
...
// ПИСАТЬ КОД ЗДЕСЬ:
let nums = [8, 1, 2, 2, 3];
let newFuncForTest = nums.map((elem) => {
    return `${elem} элемента меньше чем ${elem}`;
});
console.log(newFuncForTest);
